...
Static Function MyOpenSM0(lShared)
    ...
    // Em versões 12.1.2410, a SM0 já deve estar aberta pelo sistema.
    // Basta checar se o Alias está disponível.
    If Select("SM0") > 0
        lOpen := .T.
    Else
        // Se por algum motivo não estiver, tentamos uma abertura segura
        // sem passar caminhos de arquivo físico
        lOpen := (OpenSM0(lShared) .And. Select("SM0") > 0)
    EndIf

    If !lOpen
        MsgStop( "Não foi possível identificar a tabela de empresas (SM0).", "Erro de Ambiente" )
    EndIf
