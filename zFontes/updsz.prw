...
/*/{Protheus.doc} UPDSZ
Atualizador de dicionário refatorado para Arquitetura DB (Protheus 12.1.2410+)
/*/
User Function UPDSZ()
    Local aTabelas := {"SZ1", "SZ2", "SZ3", "SZ4", "SZ5", "SZ6", "SZ7"}
    Local nX := 0
    
    // Variáveis privadas exigidas pelas funções originais de carga (FSAtu...)
    Private aArqUpd  := {} 
    Private oProcess := MsNewProcess():New({|lEnd| AtualizaDB(@lEnd, aTabelas)}, "Dicionario de Dados", "Processando...", .F.)

    If MsgYesNo("Deseja criar as tabelas SZ1 a SZ7 no banco de dados atual?", "Atualizador do Curso")
        
        // 1. PREPARA O AMBIENTE E ABRE OS DICIONÁRIOS (Empresa 99, Filial 01)
        RpcSetEnv("99", "01")
        
        // 2. Roda as suas rotinas de atualização
        oProcess:Activate()
        
        // 3. Limpa a memória e fecha as tabelas
        RpcClearEnv()

        MsgInfo("Processo finalizado. Verifique se as tabelas foram criadas no SQL Server.", "Concluido")
    EndIf
Return NIL

Static Function AtualizaDB(lEnd, aTabelas)
    Local nX := 0

    // 1. Grava metadados (Dicionario Logico)
    oProcess:SetRegua1(8)
    
    oProcess:IncRegua1("Atualizando Arquivos (SX2)...")
    FSAtuSX2()
    
    oProcess:IncRegua1("Atualizando Campos (SX3)...")
    FSAtuSX3()
    
    oProcess:IncRegua1("Atualizando Indices (SIX)...")
    FSAtuSIX()

    // 2. Cria as tabelas fisicas no SQL Server
    oProcess:IncRegua1("Criando tabelas fisicas no banco de dados...")
    __SetX31Mode(.F.)
    For nX := 1 To Len(aTabelas)
        X31UpdTable(aTabelas[nX])
    Next nX

    // 3. Grava dependencias do sistema
    oProcess:IncRegua1("Atualizando Parametros (SX6)...")
    FSAtuSX6()
    
    oProcess:IncRegua1("Atualizando Gatilhos (SX7)...")
    FSAtuSX7()
    
    oProcess:IncRegua1("Atualizando Consultas (SXB)...")
    FSAtuSXB()
    
    oProcess:IncRegua1("Atualizando Relacionamentos (SX9)...")
    FSAtuSX9()
    
    oProcess:IncRegua1("Atualizando Helps...")
    FSAtuHlp()
